...
Заданий об’єкт user, який представляє інформацію про користувача, включаючи його ім’я, 
контактні дані та адресу. Напишіть функцію gettUserInfo(obj), яка приймає цей об’єкт obj, як параметр і 
використовує деструктурування для отримання певних деталей.
Функція повинна витягнути властивості firstName, lastName, email, phone та city з об’єкта і повернути новий об’єкт, 
що містить лише ці витягнуті властивості.
...
// значення з об'єкта або масиву і записуємо одразу в змінні.

function gettUserInfo(obj) {
    const { firstName, lastName, contact: { email, phone }, address: { city } //  деструктуруємо 
    } = obj;
...
Реалізуйте функцію countCars(cars), яка отримує на вхід масив автівок cars. Функція використовує Map() для підрахунку входжень кожного авто 
і повертає мапу з кожним авто та кількістю його входжень.
...
function countCars(cars) {
    ...
    for (let car of cars) {
        if (countMap.has(car)) {                  // .has(car) — перевіряє, чи вже є така машина в мапі
            countMap.set(car, countMap.get(car) + 1); // .get(car) — отримує поточну кількість
        } else {
            countMap.set(car, 1);                   // .set(car, value) — додає або оновлює значенняб тобто вперше бачимо таку машину — додаємо в Map з кількістю 1 
        }
    }
...
Якщо надані облікові дані збігаються, вона повинна завершити роботу з привітальним повідомленням. Якщо вони не збігаються, вона повинна 
відхилити запит з повідомленням про помилку.
...
/* Task 4. Ви розробляєте додаток для соціальних мереж, який отримує дані з двох асинхронних функцій: одна з них завантажує профіль користувача, 
а інша - його дописи. Ви хочете отримати обидві частини інформації послідовно для відображення на сторінці профілю користувача.
