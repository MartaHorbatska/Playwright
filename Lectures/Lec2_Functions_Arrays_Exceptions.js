...
// console.log(removeAll([1, 2, 3, 2, 4, 2, 5], 2)); 
// // [1, 3, 4, 5]

const days = ["mo", "tue", "we", "th", "fr"]; // додати елементи
...
function deleteAll(arr, item) { // видалити всі
    return arr.filter(el => el !== item);
}
console.log(deleteAll(["mo", "tue", "we", "we", "fr"], "we"));
// ["mo", "tue", "fr"]


// ***************не змінює ориг масив - видаляє перуш і ост
let days = ["mo", "tue", "we", "th", "fr"];   // 
function removeFirstAndLast(arr) {
arr.pop();  // видаляє останній ("fr")
arr.shift(); // видаляє перший ("mo")
return arr;
}
console.log(removeFirstAndLast(days)); 

// // *************** змінює ориг масив - видаляє перуш і ост
function removeFirstAndLastCopy(arr) {
    return arr.slice(1, -1); // повертає копію без першого і останнього
}
let days = ["mo", "tue", "we", "th", "fr"];
console.log(removeFirstAndLastCopy(days)); 
console.log(days); 
...
let input = prompt("Введіть радіус кола:");
...
try {
    ...
    alert(`Неправильне значення: "${input}": ${error.message}`); // тут ми виводимо повідомлення про помилку
}
